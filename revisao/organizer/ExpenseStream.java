...
public class ExpenseStream {
    ...
    public static void main(String[] args) {
        ...
        List<Double> expenseAmountList = expenseList.stream()
        ...
        Double totalAmount = expenseAmountList.stream()
                .reduce((double) 0, (subtotal, expenseAmount) -> subtotal + expenseAmount);
        System.out.println(totalAmount);

        System.out.println("Total por categoria de despesa FIXA: ");
        List<Double> categoryAmountList = expenseList.stream()
                .filter(expense -> expense.getCategory() == "Fixa")
                .map(expense -> expense.getAmount())
                .collect(Collectors.toList());

        Double categoryAmount = categoryAmountList.stream()
                .reduce((double) 0, (subtotal, expenseAmount) -> subtotal + expenseAmount);
